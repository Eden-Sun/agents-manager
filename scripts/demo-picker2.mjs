...
const shot = async (name) => { await sleep(350); const { data } = await send('Page.captureScreenshot',{format:'png',clip:{x:0,y:0,width:460,height:620,scale:2}}); writeFileSync(`${OUT}/${name}.png`, Buffer.from(data,'base64')); console.log('saved', name) }
...
await shot('301-picker2-selected-row')
...
await state('kbd')
await shot('303-picker2-keyboard')
