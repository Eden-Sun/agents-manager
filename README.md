...
bot 對 bot 送訊息的慣例：走 `POST /api/bots/{id}/prompt` 的要帶 `relay_from=<自己的 bot_id>` 與 `-H "X-AM-Bot-Token: $AM_HOOK_TOKEN"`（省略 `relay_from`＝使用者自己打的，不存在的值回 400；token 不是自己的 403；沒帶 token 相容期照收、標「未驗證」；`relay_from=daemon` 從 API 帶進來 403）；走 `herdr agent prompt` 的由 PATH shim 經 `/relay/announce` 認領，第一行再順手寫 `[來自 …]` 抬頭當備援。規格見 [`docs/API.md`](docs/API.md) 的 prompt 一節。
...
`scripts/check.sh`，讓本地檢查與 CI 使用同一組指令；`web`／`daemon` 參數可只跑一側。
