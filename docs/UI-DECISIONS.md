...
- **手機主力區：4 顆一排的 grid、最多兩排、不橫捲（2026-09-20，#344 追加）**：手機（≤720px）的主力區不再是橫捲列，改成 `grid-template-columns: repeat(4, 1fr)`、往下換行、收合時兩排——前幾顆由使用者用拖曳決定（`lib/pinnedOrder.ts::PIN_GRID_MAX`）。
  **超過 8 顆時第 8 格是「+N」**（2026-09-23 使用者：「手機版 header 主力只兩排，多的呢」——原本多的直接不畫，完全找不到）：
  收合時畫前 7 顆＋「+N」，點了展開全部（多幾排），最後一格變「收合」；+N 是虛線框，藏起來的有要回答／未讀時吃同一組狀態色，
  tooltip 照桌機 `+N` 的規則寫出藏了什麼（`pinGridLayout`）。截圖：`docs/screenshots/primary-order/grid-more-*.png`。
