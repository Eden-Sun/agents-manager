...
- **送出的收尾只動自己那顆 bot**（2026-09-24，issue #496）：托盤住在 `ChatPanel`、切 bot 只清內容不重建，所以送到一半切走時，那一則的收尾不清托盤（清到的會是新那顆剛加的附件，連上傳都被 abort）；輸入框的「送出中」也跟著新 bot 重來（`<Composer key={botId}>`），不會把別顆的輸入框鎖住。
