...
- 未讀：`!N` 方角小標（不是圓點），單位是回合；專案收合時加總掛到標題；分頁標題 `(N)` 只算 bot 那份；已讀標記＋數字存 localStorage，刪 bot 時清帳。
...
- 更新 chip 只寫 `${kind} 有更新`；批次重啟蓋得到當前 bot 時不畫（忙碌、非 user 管理、非 claude 才留）。
...
- 桌機：依緊急度排序（要回答 → 未讀 → 等子 agent → 正在看 → 其餘），最多兩行，其餘收進 `+N`。
- 手機：單行橫捲、**位置固定照側欄順序，不照緊急度重排**（使用者指定），有 ◂ ▸ 與 scroll shadow，正在看的那顆自動捲進畫面。
...
- `limit_hit`（CLI 擋住）：只留名字前一顆紅點＋淡紅外框，量表與數字灰階退色；kind logo 永遠是品牌色（嚴重程度不畫在 logo 上）。
...
- 認得出選單時（`lib/tuiChoices.ts`：至少兩項、連號、剛好一個游標、在畫面底部；認不出就整塊不長、退回終端快照＋按鍵面板）：
  ...
  - `Type something.`：選到就展開多行輸入，送出時 paste → 對帳 → Enter。
...
- 圖片暫存托盤：右緣可收合窄欄（≤1024px 在底部）；點一下放進目前對話（拖放是加分）；複製不搬移；存 IndexedDB、30 分鐘後清；
...
- 主機 shell：bot 標題列底下第三個分頁（沒選 bot 時整頁）；一行輸入＋一排按鍵、↑↓ 是指令歷史；「清畫面」真的送 `clear`；
  「關閉」與「結束 shell」分開（後者二次確認）；草稿與畫面跨重新整理保留。
...
- 手機：輸入框不被壓扁、上限跟可視高度走；對話框一律全螢幕 sheet；抽屜選完就收，但從抽屜開對話框時不收。
  bot 標題列第一行 ☰、名字、★、⚙、分頁，第二行 kind＋模型＋版本；git 鍵疊在額度列左下，跟齒輪同一種灰。
- 桌機可在右欄嵌手機預覽（環境設定 → 顯示，預設關）：iframe 帶 `?mobilePreview=1`、393×852 以 `scale(.5)` 顯示（不改 iframe 尺寸）。
