...
同日再補：週窗口**不必等到 0%，剩不到 10% 就寫倒數**（「低於 10% 也 show」）——快見底時該看的也是還要撐多久；百分比移到滑鼠提示（`剩 9%，還有 4h00 重置`），量表條本身仍看得出剩多少。5h 仍是用完才寫。門檻在 `lib/quotaReset.ts`（`RESET_SOON_MS`／`RESET_SOON_WEEKLY_MS`）。
