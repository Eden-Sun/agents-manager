...
## 檔案暫存的下半段＝bot 的 scratchpad（2026-09-16，使用者指定）

「scratchpad 讓我可以在檔案暫存裡點選下載」。bot 常把整理好的東西寫成檔案（`tracking.tsv`、報告 `.md`），以前那些只在 daemon 那台機器的暫存目錄裡，手機上拿不到。

- **同一個托盤、兩個方向**：上半段是拖進來、要**送進**對話的暫存（IndexedDB、30 分鐘後清）；下半段是**從** bot 拿出來的檔案。所以下半段點一下是下載，不是放進對話。
- **唯讀**：不刪檔、不改檔、沒有上傳。bot 的工作目錄不該被面板改掉。
- **跟著選取的 bot 走**，不是跟著專案：scratchpad 是每個 session 一份。換 bot 在 render 當下就清空清單，不然中間那一格會先畫出上一顆 bot 的檔名。
- **不自動輪詢，給一顆重整**：bot 寫檔的時機不可預測，每秒去掃目錄只是白費。標題列那顆 ↻ 按了才重讀。
- **列不出來要講是哪一種**（遠端主機／還沒跑過／目錄還沒建立），不要留一片空白讓使用者以為按錯了；這幾種都不是紅字錯誤。
- **一律附件下載**：daemon 端 `Content-Disposition: attachment` + `nosniff`，白名單外的型別一律 `application/octet-stream`。bot 寫出來的 HTML 不該在使用者的 daemon 網域裡被執行。
- **預設只列對話裡提過檔名的**（使用者 2026-09-16：「沒提到過的檔案怎麼也在這裡面」）：scratchpad 是 bot 的工作桌，大部分是它自己跑過的腳本與中間產物（實測 AM-1-L 54 個檔只有 1 個在對話裡提過）。其餘收在「▸ 其他 N 個（對話沒提到）」，預設收起、換 bot 就收回去；不是刪掉，bot 交代時沒寫檔名也還找得到。標題列的數字只算提過的。
- **資料庫、金鑰一律不列也不給下載**：實測 scratchpad 裡有正式 DB 的完整複本（`*.sqlite3`、`*.db` 帶 `-wal`/`-shm`）和 `.pem`。那些不是「交給使用者的東西」，而且拿到一個 UI token 就能整份帶走。判斷在 daemon，除了看檔名也看檔頭（改過副檔名的 SQLite、PEM 私鑰）。
- **最高佔暫存欄的 60%**（使用者 2026-09-16）：檔案兩三百個時整欄會被它吃掉，上半段的暫存就看不到。超過就在清單裡捲。641–1024px 的底部暫存條只有 `max-height: 46vh`（不是確定高度，百分比失效），改用 `46vh × 60%`；≤640px 本來就不顯示暫存區。
