...
- 未讀：`!N` 方角小標（不是圓點），單位是回合；專案收合時加總掛到標題；分頁標題 `(N)` 只算 bot 那份；已讀標記＋數字存 localStorage，刪 bot 時清帳；**bot 的已讀標記與未讀數以 daemon 為準、跨裝置共用**（2026-09-15：手機分頁凍結收不到回合完成、在桌機讀過手機不知道，兩邊主力區不一致），本機標記與 daemon 標記取較新者。群組未讀仍是各瀏覽器自己的。
...
- **「不再出現」是字面意思**（使用者 2026-09-16）：標題列額度條、快速新增 Bot、Bot 設定的身份選單、側欄的身分計數、群組任務挑身分全都濾掉，連它自己那把孤兒額度 key 也不列；daemon 也不再花探測去問它的額度（還有 run 在跑的例外）。只有環境設定這一頁留著它，否則沒有地方把它按回來。
...
- **開關而不是預設**：同步開著時終端吃掉鍵盤，這對「只是想看一眼輸出」的人是干擾。預設關，開關狀態每個 pane 各記一份。
...
## LAN 要配對才進得來（2026-09-16，AGM 裁示）

`GET /api/session` 是唯一不需 token 的端點，而它發出去的那把 token 過得了 `auth()`＝整個 API（往任何 bot 的 TUI 打字、開主機 shell、kill 程序、刪專案）。`allow_lan` 打開時 daemon 綁 `0.0.0.0`，同網段——**含 Tailscale 這種疊加網路，等於跨網際網路可達**——任何裝置一個 `curl http://<ip>:7788/api/session` 就拿到整把鑰匙；2026-09-16 實測拿得到。但使用者的手機走的就是這條路，所以不能把門直接關上，改成出示一次性配對碼換 token（SPEC §7.1a）。前端這一側的取捨：

- **沒配對過看到的是輸入框，不是錯誤畫面**。daemon 回的是 403，但 daemon 其實好好的——在手機上顯示「無法連上 daemon」只會讓人去重開 daemon。只有 `403 pairing_required` 走配對畫面；同樣是 403 的 `non-local request`（Host／Origin 不對）仍走錯誤畫面，那是設定問題。
- **產碼只能在本機**（`POST /api/session/pair-code` 要 token 且只收 loopback）：碼是用來把整個 API 的權限交出去的，按下它的人必須已經站在這台機器前面。在手機上開同一頁按下去會收到 `loopback_only`，畫面要好好講「只有在這台機器上的瀏覽器才產得出來」，不是丟一行紅字。按鈕本身仍然留著——藏起來的話，使用者會照著 daemon 的錯誤訊息去找一個看不到的東西。
- **已經配對過的裝置不會被踢出去**：token 存這台裝置的 localStorage，有存過就優先用，連 `/api/session` 都不問（問了也只會再拿到一次 403）。收到 401 才清掉它、重新取得一次——**只重試一次**，第二次還是 401 就照實報錯，不然一把壞掉的 token 會變成打不完的迴圈。重新取得時又被要求配對，就回到配對畫面。
- **碼的正規化跟 daemon 同一套**：大小寫、空白、連字號都不算，使用者照著唸出來打就好。輸入框**不**邊打邊改成大寫或自己塞連字號——游標會亂跳、也很難刪掉。
- **兩種倒數兩種說法**：429 的等待講白話（一分鐘以上換算成「N 分鐘」，無條件進位；daemon 鎖十分鐘，「請等 600 秒」沒人讀得動），碼的到期倒數則要看得到秒在動（五分鐘的碼寫「5 分鐘」會整整五分鐘都不變，看不出還來不來得及走到手機前面）。兩個倒數都以 `expires_at` 重算，不自己減一：分頁切到背景時計時器會被凍住。
- **碼不對／過期／用過只有一句話**：daemon 對外就是同一種回答（不透露是哪一種），前端不要自己猜著補充。
