...
多分頁 ＋ 多選（claude 一次問好幾題的 AskUserQuestion）也吃：上面長一條分頁列（☑ 答過、
✔ 送出頁，「現在在哪一題」快照讀不出來，UI 猜第一個未答的並畫成虛線框），選項前面是 ☐／☑，
底下多一顆「送出（Submit）」。多選點一下是**切換勾選**：走到那一列 → 試 `space` → 試數字鍵，
看 `[ ]` 有沒有變成 `[x]` 來判斷成功，**一次都不按 Enter**（Enter 可能是交卷）。Enter 只在單選、
以及確認游標停在 `Submit` 上之後才送。

**兩個分頁以上的問卷走草稿模式**（`lib/choiceDraft.ts` ＋ `BlockedDraft.tsx`）：先用 ←／→ 把每頁
讀回來（只送導覽鍵、走完回起點），之後勾選與跳題全部只改本地 state 不碰終端，按「送出全部答案」
才照差集一頁一頁送並驗過去，任何一步對不上就停手。預載失敗就退回即時模式。

選項的說明**預設收起來**（一列一行，放不下截尾），按列尾那顆 ▸ 才展開那一項——它是整列按鈕的
兄弟不是子元素（`<button>` 不能巢狀），點它不送任何鍵。收合／展開前後用 `scrollTop` 補回那一列
的位置，畫面不會跳；游標換項時 `scrollIntoView({ block: 'nearest' })` 把當前項帶進視野。
