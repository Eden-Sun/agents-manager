...
- worker 的 persona 明說：cwd 就是你的 worktree、不得 `cd ..`、不得動 `../main` / `../dev-2`、不得進入 `<project.path>`（使用者的 checkout）、不得 `git push`、不得改分支、禁止 `git stash` / `--autostash`、只 `git add` 自己的檔案、收尾前跑 `scripts/am-dev-check.sh`、每個邏輯段落 commit。這些是「指示」不是「強制」；強制層是 worktree（改錯地方也只汙染自己的 worktree）與 daemon 只合併它建的那條分支。
...
| 成員 `blocked`（trust 提示、權限詢問） | `paused(member_blocked:<name>)`；沿用 blocked 面板送鍵；狀態離開 blocked 時 **自動 resume**（唯一會自動 resume 的原因，因為它不是預算問題）。 |
...
- persona：「你是 <短名>。你的 cwd `<path>` 是專屬 worktree，你只能在這裡工作：不要 `cd` 出去、不要動 `../`、不要 `git push`、不要切換分支。禁止 `git stash` / `--autostash`，只 `git add` 自己的檔案，收尾前跑 `scripts/am-dev-check.sh`。每個邏輯段落 `git commit`。完成後用 `report` 回報：`summary` 說明改了什麼、如何驗證。做不下去用 `status: blocked` 說明原因。」
