...
- `delivery = unknown` 時禁止再送 prompt，只允許 `interrupt`、`stop` 或 `POST /turns/:id/abandon`——否則下一則 hook 會配錯回合。
  hook 回報回合結束時照舊認領這顆 run 唯一的 in-flight turn 並把 `unknown` 升成 `ok`，**除非** hook 看得到這一回合的使用者訊息（codex 直接帶 `input-messages`；claude 從 `transcript_path` 尾巴讀最後一則），而且跟這筆的 `prompt_text` 拿掉**所有**空白後互不包含（不是收成一個空格：送進 TUI 時 Tab 會被吃掉，`進貨單\t每張` 到了 transcript 是 `進貨單每張`，2026-09-27 wits-pro）——那是使用者在終端手打的另一句：不認領、記成一筆外部回合，原本那筆維持 `in_flight`／`unknown`（第二輪 review 送達線 #3）。讀不到使用者訊息時不下判斷。
...
（身分、桶、撞限時刻、到期、原因、寫下的時刻、開機代號 `App::boot_id`，以及那台主機當時指到的機器 `target`＝ssh 目標:port/herdr session）
寫進 `turns.quota_hold`，閘門放行就清掉。**上一輪開機**寫的憑據，
在那台主機的開機回填跑完之前（身分表還沒進來，`cc0` 這類 key 算不準）flush 直接看它：身分沒換、撞的桶管得到現在的模型、還沒到期、
寫下之後同一把 key 沒被成功回合清過（`quota::limit_cleared_since`）、**主機名還指到同一台**（`target` 跟現在的設定一樣；#347），就擋。
改指到另一台之後那是別台機器上的帳號，回填也不種回；單純重連不改設定，照樣算數。主機不在設定裡不算「換了」（讀不到不放行），
舊版寫的憑據沒有 `target` 照舊算數。排隊保險絲判「還在被額度擋」用的是 flush 同一支
