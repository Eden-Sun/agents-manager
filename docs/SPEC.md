...
1. 框必須是空的才開始打。框裡的內容**跟整段逐行相同**（含縮排與硬換行）才算我們自己上次留下的，
   直接送出、不重打；
   只看得到一截（多行只落第一行、框被捲動、TUI 截斷、或使用者草稿剛好引用了任務片段）一律回
   `Unknown`，既不覆蓋也不按 Enter；是別人的字同樣停手，不清框。
2. `pane.send_text` 之後必須在框裡看到這段文字（比對去空白後的頭、尾或整段片段，折行與中文都認得）；
   框證明是空的（什麼都沒落地）才重打一次，其他情況一律不重貼。
3. Enter 之後框要空、而且 agent 在**對話區**多印一次這段文字，才算送出。回音以**整塊**解析（標記列
   加它的續行＝一則，多行 prompt 不會被拆成多則），還原後要與原文**逐行相同**才算；被 `…` 截斷、
   只回音第一行、縮排被改掉都不算。

   還原的規則：只拿掉**可證明是終端機自己加的**空白——續行前面那兩欄裝飾縮排，以及「上一列已經排到
   行尾」時的那個換行（軟折行；pane 寬度取畫面上滿版的 `───` 規則列，顯示寬度 CJK 算兩欄）。prompt
   自己的硬換行與行內縮排一律保留，所以縮排被弄壞的程式碼不會被當成同一段。量不到寬度又有可能折行的
   列就是還原不了，回 `Unknown`。spinner、計時、狀態列刷新一律不是證據，短 prompt 也要有自己的回音行。
   第二次 Enter 同樣要驗。
4. 任何一步讀不到畫面就是錯誤，不是空畫面；驗不出來回 `Unknown`，turn 停在 `delivery='unknown'`（§6.3），
   ...
   可打又不能信任 agent 時回 `Unknown`，不退回 `agent.prompt`。**寫入先於打字**：`runs.pane_typed`
   要先寫成功才動 pane（slash 與 prompt 都是），寫不進去就中止這次操作——打了卻沒記住，下一則與重啟
   後又會走回會吞訊息的路；行程內另有保守記號，本次啟動內不會忘。
