...
24. 確保開發用 dev server（`web/` 的 vite，http://127.0.0.1:5173）一直在跑：launchd `com.agm.dev-server` 每 5 分鐘跑 `bin/dev-server-kick.sh`，有回應就什麼都不做，沒回應就在 `web/` 以 `bun run dev --host 127.0.0.1 --port 5173 --strictPort` 拉起，log 在 `dev-server.log`；port 被別的程序占著時只寫 log、不 kill。AGM 不用 LLM 巡邏這件事。5188 等其他 port 的 vite 是各 bot 自己的測試實例，不歸這條管。
