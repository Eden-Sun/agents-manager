...
27. 收到通知就 `bin/agm ack`：沒有 ack 的通知 daemon 只會補送 5 次（或開著 6 小時）就停手，改推一則 `inbox_gave_up` 叫醒協調者請它看你怎麼了——
    ...
    收到 `inbox_gave_up`（`owner_role=responder`）或 incident `role_unavailable`／`responder_undeliverable` 時，先用 `bin/agm responder show` 看協調者的 `unavailable_reason` 並讀它的畫面。`role_unavailable` 的**故障對象看 `resource`**（你收到的一定是 `resource=responder`，`resource=patrol` 那筆會送給協調者）、**原因看 `detail.reason`**。判斷一律讀 `unavailable_reason` 這一欄，不要去解析 `status`：`notify_stalled`（送得出去但一直沒被收下）從來不會寫進 `status`，只看 `status` 的話那個原因永遠看不到，而 `waiting_quota`／`no_run` 的 `status` 本來就比 `unavailable` 好懂。`needs_login`（畫面寫 `Not logged in · Please run /login`）就**直接推通知給使用者**，寫明哪顆 pane、要跑 `/login`（Keychain 鎖住時先 `security unlock-keychain`），以及有哪些申請在等；不要只在自己的對話裡問一句等人回覆。
...
8. 派 issue 相關的工作之前先 `bin/agm issue claim <n> --child <名> --worktree <p> --branch <b>` 認領：exit 3 代表別的 bot 正在做，**不要再派第二顆**，照它印的 `claimed_by` 去協調（24 小時沒動靜的認領才可接手）；child 收尾或決定不做時 `bin/agm issue release <n>`（issue #425）。派工先建立持久 assignment，使用穩定 client_request_id，保存目標 Bot、turn 與來源。重試沿用同一請求 ID；delivery unknown、延遲回報或重複交辦先對帳。一般忙碌 Bot 等當前回合完成，不中斷或往模型選單塞訊息。
...
25. 群組任務（使用者在群組裡「交給 AGM」）依 SPEC §18.14 的 runbook 調度：規劃→執行者→reviewer→驗證者→交付→回報，每步一件交辦掛在 mission 上，身分由 daemon 的 pick 決定（cc2→cc1→cc0、驗證者必須 Fable）。輪數上限、驗證者沒 Fable、交付非 fast-forward、指示不清，一律停下在群組問使用者一個具體問題，不自行降級或硬推；撞額度換手由 mission_identity_switch 觸發，用新身分開 bot 對原交辦 followup 接手。
