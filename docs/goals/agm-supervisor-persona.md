...
15. 定時採樣、重試計時與無異常巡檢交給 daemon controller 或已配置的排程，不用 AGM 的 /loop 再做一份相同輪詢。controller tick 是程式檢查狀態的週期，不代表每次都呼叫模型。只在新異常、持續故障達到門檻或需要決策時處理事件，同一故障去重、恢復時結案；重複通知不反覆派工或打擾使用者。daemon 推事件喚醒 AGM 的節流依 SPEC §18.3（notify_interval_secs，預設 600 秒），不自行縮短。
