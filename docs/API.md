...
`/api` 的 `auth` 中介層驗 `Origin` 與單一 principal。網頁繼續用共用 User token；Bot 呼叫帶 `X-AM-Bot-Id`＋該 bot 的 token；兩個 service 只可呼叫明列的路徑。`X-AM-Caller` 只當自稱記進 config 寫入 log。需要角色閘門的操作再檢查已驗證的 Bot id 是否為巡檢或協調者；需要明確確認的操作仍用 `?confirm=<字>`。
...
```
  ...
  |---|---|
  | User，省略 | 使用者本人 |
  | Bot，省略或等於自己的 Bot id | 記成已驗證的 Bot；若等於收件 bot 本身，回 **400** `relay_self` |
...
- `POST /api/release-triage/publish {kind?, version?}` → `{publish_enabled, results:[{kind,version,result:{outcome:disabled|published|deferred|failed,…}}]}`。重試所有（或指定的）`judged` 版本；`publish = false` 時每一筆都是 `disabled`、gh 不會被呼叫。`kind` 不在 `claude|codex` 回 400，`version` 會正規化（`v0.156.0` 與 `0.156.0` 等價），認不出來回 400——不再靜靜回空（#458）。
- `POST /api/release-triage/publish {kind?, version?, dry_run:true}` → `{dry_run, publish_enabled, checks:{repo,gh_auth_ok,gh_auth_error,repo_ok,repo_error,viewer_permission,can_write,issues_enabled,labels_missing[],remote_list_error?}, would_create, would_comment, existing, blocked_by_caps, versions:[{kind,version,proposals:[{marker,triage,entry_ids,title,body,labels,action,writes,number?,url?,error?}]}]}`。
...
- `GET /api/supervisor/incidents?all=0|1` → `{incidents:[{id,kind,resource,severity,status,detail,occurrences,first_seen_at,last_seen_at,resolved_at}],open,all}`。
  `kind`：`host_disconnected` | `bot_stopped` | `assignment_stalled` | `assignment_undelivered` | `notify_exhausted` | `role_unavailable` | `responder_undeliverable` | `approval_stalled` | `inbox_classify_failing` | `remote_shim_stale`（SPEC §18.9／§18.15；`inbox_classify_failing` 是 issue #472——`roles::classify` 連續 3 拍失敗，critical，resource 固定是 `inbox`，detail 帶 `consecutive_failures`／`action`；**這一種不入 inbox**，理由見下一行；`responder_undeliverable` 是 issue #420，`approval_stalled` 是 issue #421——核准開著 30 分鐘沒有任何人裁示，critical，resource 是核准 id，detail 帶 `approval_id`／`waiting_secs`／`requester`／`action`；`remote_shim_stale` 是 issue #534——遠端 shim 補版退避重試（立刻／5 分／15 分／60 分）都失敗、或有檔案換不動，degraded，resource 是 host 名，detail 帶 `why`／`action`，補成就自動恢復）。一個 resource 同時只有一筆 open；開啟與恢復各推 inbox `incident_opened` / `incident_resolved`。
  **收件人不一定是巡檢**：`notify_exhausted` 推給協調者（倒下的正是巡檢的通知路），`role_unavailable` 看故障的是誰——`resource='patrol'` 推協調者、`resource='responder'` 推巡檢；其餘都推巡檢。
  `inbox_classify_failing` 誰都不推：壞掉的就是「把事件分給角色」那一步，推進去的那一則同樣會停在 `role IS NULL`、同樣沒有人收得到（用壞掉的路通報那條路壞了）。只在這支與 `system_health` 看得到。
...
- `PUT /api/supervisor/persona {text,expected_version?}` → 同上。正文改變才版本 +1，並改寫 config.toml 的 bot persona 與 `persona.md`（副本，不要手改）。
  **身分（issue #462、#556）**：User principal 照收（網頁使用共用 UI token）；有效 Bot principal 且是角色 bot → 照收；有效 Bot principal、**那顆不是角色 bot** → 403 `role_required`；Bot／service 身分標頭不完整、憑證錯誤或混帶其他 principal → 全域中介層 401，不退回 User。
  ...
  `#556` 保留共用 UI token：沒有 Bot／service header 且通過 UI token 驗證就是 User。Bot header 一旦出現會驗 per-bot token，驗證失敗不降級；這不提供額外的人類證明。
...
- `POST /api/supervisor/assignments {target_bot_id,text,client_request_id,source_turn_id?,ownership?:[path],kind?:"task"|"notice",expects_review?,mission_id?,role?,review_role?:"patrol"|"responder"}` → 一筆 assignment（多 `review_role`）：
  ...
  - **長度**：`text` 超過 200,000 字 → 422 `{error:"text_too_long",max_chars,chars,sent:false}`，不建列（跟 bot prompt API 同一個上限；以前收下後才在派送時以 `prompt_too_long_to_prove` 靜默失敗）。`agm` 在送出前先量並回 `bad_args`。
  ...
  - `attempts` 與 `busy_rounds` 不是同一件事（#528）：`attempts` 是上游／暫時性錯誤重試到第幾次（用完才 `dispatch_failed`），`busy_rounds` 是 409（對方回合中、沒有 run…）排不進去幾輪，不吃 `attempts` 的額度。
  - 一直送不進去（連續 409）：從 `conflict_since`（這一輪第一次 409；進 `quota_blocked`、被窗口 hold、送達時清掉）起超過 `AM_DISPATCH_CONFLICT_GIVE_UP_MINS`（預設 30 分），或 `busy_rounds` 到 20 輪，改成 `blocked`＋inbox 的 `assignment_undeliverable`（payload 帶 `busy_rounds`／`max_busy_rounds`）
...
- `GET /api/supervisor/inbox?all=0|1&limit=200&role=patrol|responder` → `{events:[{id,event_key,assignment_id,bot_id,turn_id,kind,payload,state,notify:{turn_id,delivery,attempts,next_at,error,delivered_at},role,wake,claimed_by,acked_by,merged_into,created_at,updated_at}],open,all,limit,role}`。
  ...
  **只有 AGM 角色結得掉**（issue #432）：User principal 或有效但不是角色 bot 的 Bot → 403 `role_required`；service principal 因不在此路徑授權範圍也會被拒絕；Bot／service 身分不完整、憑證錯誤或混帶 principal 由全域中介層回 401。
...
**角色標頭是三態，不是兩態（issue #415）**：沒有 Bot／service header 且有有效 `X-AM-Token` = User；帶了成對 Bot headers 且 token 對得上 = 那顆已驗證 bot，再由各端點判斷它是否是角色 bot；
帶了卻缺欄、token 空／非 UTF-8／不符，或混帶其他 principal = 全域中介層回 401，**不會**退回 User 權限。
以前最後一種也當成「沒帶」，而沒帶在 `inbox/{id}/ack` 是 `1=1`（什麼角色的事件都結得掉），於是角色 bot 把自己的 token 打壞反而比帶對還多權限。
`inbox/{id}/ack` 那一支在 issue #432 之後連未帶角色身分也不收（403 `role_required`）；其他需要角色的端點照各自守衛處理。沒有 Bot／service header 且帶有效共用 UI token 仍是 User，這是使用者接受的憑證界線，不是人類證明。
...
`relay_from`：`null` = User principal（只有 `instruction`），bot id = 那顆 Bot principal，`"daemon"` = daemon 自己記的。

**mission 的 `relay_from`**（`events`／`question`／`answer`／`revise`／`complete`／`deliver`，issue #409，SPEC §6.5d 第 6 點）：跟 §5 共用同一段 token 比對，擋下來就什麼都不寫。

| `relay_from` | 條件 | 回應 |
|---|---|---|
| 省略 | User principal | 使用者本人 |
| 省略／空白 | Bot principal | daemon 以已驗證的 `X-AM-Bot-Id` 標記 |
| bot | 同一顆 Bot principal，且 `X-AM-Bot-Token` 有效 | 照收 |
| bot | Bot principal 的 id 與 `relay_from` 不同，或 User principal 有效帶 token 但指到不存在／已刪的 bot | **403** `relay_from_mismatch` |
| live bot | User principal 沒帶 Bot proof | **403** `relay_from_token_required`（沒有相容期） |
| 不存在／已刪 bot | User principal 沒帶 Bot proof | 400 |
| 不存在／已刪的 bot | 沒帶 | 400 |
| `"daemon"` | 呼叫端是驗證過的 AGM 角色 bot（`X-AM-Bot-Id`＋`X-AM-Bot-Token`，`agm mission … --as-daemon`） | 照收 |
...
| POST | `/api/missions/{id}/events` | `{kind: "report"\|"note"\|"verified", text, relay_from?, payload?, worktree?, sha?}` → 事件。`relay_from` 規則見下方「mission 的 `relay_from`」。**交付前必須有一則 `verified`**，而且 `verified` 要說驗的是哪個 commit：`worktree`（本專案 repo 的工作樹，daemon 讀它的 HEAD）或 `sha`（可縮寫，必須是本專案 repo 裡的 commit），兩個都給時必須一致；daemon 把完整 sha 寫進 `payload.sha`。都沒給、工作樹不是本專案的 repo、sha 找不到 → 400。寫入時在同一個交易裡重看（驗 commit、算代都在交易外）：任務已結案 → 409 `already_closed`；`verified` 算好代之後任務被退回（`round` 先落地）→ 409 `verification_stale`（`stale_because: round`、`verified_generation`、`generation`、`next`）——驗的是上一代的成果，落地卻會被算成新一代的驗證。兩種都什麼都不寫。 |
