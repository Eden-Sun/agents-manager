...
pub const RUN_STATE_EDGES: &[(&str, &str, &str)] = &[
    ...
    ("exited", "stopped", "lifecycle/stop.rs commit_stopped：stop 自己關的 pane 觸發的 pane-exit 事件搶先寫了 exited，改標成使用者要的 stopped（兩個都是終態、不復活；收尾那條路做過了，不做第二份，#146）。run_state::relabel（CAS＋同一顆 bot 沒有別的 active run）；只有使用者的 stop 改標，重啟那一半不改；寫不進去回 503、排 FinishStop 重試（#146 重開）"),
    ("stopped", "exited", "lifecycle/start.rs left_down_by_restart：重啟停掉了卻沒開回來。run_state::relabel（同上）；寫不進去回 503 restart_state_uncommitted、排 Relabel 重試（#146 重開）"),
